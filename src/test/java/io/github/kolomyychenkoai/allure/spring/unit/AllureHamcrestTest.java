...
class AllureHamcrestTest {
    ...
    @Test
    @DisplayName("несовпадение шага НЕ создаёт (падение покажет Allure)")
    void failedMatchProducesNoStep() {
        // внутри allure.run — только действие под тестом; ожидание исключения через нейтральный
        // JUnit assertThrows (НЕ инструментируется), иначе AssertJ сам залогировал бы шаг в активный кейс
