 ...
 * порты ({@code @<HASH>}, {@code (:<PORT>)}), то есть частично ПРЯЧЕТ ровно то, что здесь ищется.
 * <p>
 * Страж ОБЩИЙ, а не помодульный: мусор рождается в единой точке рендера значений
 * ({@code AllureAdviceSupport.safe}) и течёт во все модули сразу.
 * <p>
 * Нарушителей ноль, глушилок нет ни одной — гейт держится «в зелёном» по всем именам прогона.
 * Появился мусор — чинить рендер в {@code src/main}, а не добавлять исключение сюда.
 */
