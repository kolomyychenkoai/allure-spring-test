 ...
 * тот же путь к тому же {@code toString()}. Тест стоит в ОБЩЕЙ точке рендера
 ...
 * <p>
 * ⚠️ <b>Защищено ЗНАЧЕНИЕ, а не всё, что внутри него.</b> Рендер спрашивает тест про само
 ...
 * коллекции ради проверки нельзя — это ровно тот побочный эффект, от которого тест и стоит.
...
public final class JpaLaziness {
     ...
     * <p>
     * {@link ClassValue}, а НЕ {@code Map<Class<?>, …>}: тест зовут на каждое отрендеренное
