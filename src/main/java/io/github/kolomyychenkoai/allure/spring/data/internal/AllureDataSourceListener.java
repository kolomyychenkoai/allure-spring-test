...
public class AllureDataSourceListener implements QueryExecutionListener {
     ...
     * <p>
     * ⚠️ Виды SQL стерегутся кратностью ({@code ×N} в эталоне инвентаря), а потолок упирает
     * ЧИСЛО шагов — то есть на пакете больше 50 запросов ОДНОГО вида кратность насытится на 50
     * вместо настоящего числа. Сегодня это недостижимо (в витрине пакет из двух запросов), но
     * если витрину когда-нибудь раздуют — сначала подними потолок, потом пересевай маркеры.
     */
