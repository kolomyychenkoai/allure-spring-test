 ...
 * <p>
 * <b>Зачем список, а не имя.</b> Автоконфиг, скомпилированный против КОНКРЕТНОГО имени, не может
 * обслуживать оба мажора: собранный под Boot 4 он у потребителя на Boot 3 МОЛЧА выключает модуль
 * ({@code @ConditionalOnClass} читается ASM-ом, класса нет — условие ложно, автоконфиг не
 * применяется, ошибок ноль). У WebTestClient это полная потеря шагов: байткод-фолбэка, как
 * у MockMvc, у него нет.
 * <p>
 * Поэтому интерфейс поднимается ПО ИМЕНИ из этого списка — один и тот же jar работает на 3.x
 * и на 4.x. ⚠️ Это снимает привязку ТОЛЬКО для кастомайзеров: другие переезды Spring правок
 * в {@code src/main} требовать могут (так, в Spring 7 {@code HttpHeaders} перестал быть
 * {@code Map}, и {@code rest/internal/AllureRestTemplateInterceptor} пришлось менять).
 * <p>
