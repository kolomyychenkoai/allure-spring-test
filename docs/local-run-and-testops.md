...
- **Сборка падает на компиляции / странные ошибки версии.**
  Библиотека собрана под **Java 25** — на сервисе с Java 21 и ниже не заведётся вовсе
  (`UnsupportedClassVersionError`). Проверь `java -version`.
...
1. основной (`default-test`) — все тесты, как обычно;
...
Если обновление УДАЛЯЕТ виды, оно тоже не пройдёт — нужен ещё `-Dinventory.remove=true`.
Если разъехалась КРАТНОСТЬ (маркеры `×N`), нужен `-Dinventory.counts=true` — он пересевает
маркеры по замеру и печатает, у каких видов кратность изменилась.
Так красную сборку нельзя «починить», молча сократив сетку: удаляемые виды печатаются списком.

Полезное:

- `mvn test -Dtest=KafkaReportIT` — точечный прогон; инвентарь при этом НЕ запускается
  (профиль `report-inventory` активен только когда свойство `test` не задано), эталон не трогает.
- `-Dinventory.strict=true` — падать и на НОВЫХ видах (для релизного прогона; в обычном
  новые виды только печатаются). Появление НОВОГО класса-витрины роняет прогон всегда:
  модуль вне эталона — это модуль, который никто не стережёт.
- `target/allure-results` чистится перед каждым прогоном (`maven-clean-plugin`, execution
  ...
  прошлых прогонов. **Исключения:** прямой вызов `mvn surefire:test` (без жизненного цикла)
  и запуск тестов **из IDE** своим раннером — там каталог не чистится, старьё копится.
- Если основной прогон КРАСНЫЙ, до инвентаря дело не доходит (Maven останавливается).
  Один флакующий тест скрывает диагноз инвентаря — сначала чини основной прогон,
  либо гоняй с `-Dmaven.test.failure.ignore=true`, чтобы увидеть обе картины сразу.

**Про апгрейд Java/Spring.** Поломку ловят три независимых сигнала, держать все три:

| Сигнал | Отвечает на вопрос | Где смотреть |
|---|---|---|
| `InstrumentationApiCanaryTest` | API чужой библиотеки уехал? | падение канарейки с именем матчера |
| `InstrumentationDiagnostics` | перехват упал с ошибкой? | `target/instrumentation-diagnostics.txt`, проверяется инвентарём |
| инвентарь отчёта | шаг исчез, а ошибок не было? | `ReportInventoryCheck` |

Третий — самый частый случай при апгрейде: матчер просто перестал совпадать.

Библиотека собирается под **Java 25** (`maven.compiler.release=25`). Версия закреплена за
проектом файлом `.java-version` (jenv), глобальный дефолт машины остался 21 — другие проекты
это не задевает.

Две разные поломки и два разных сигнала:

- **JDK СТАРШЕ нужной** → `error: release version 25 not supported`. По самому сообщению
  причину не найти, поэтому `scripts/compat-matrix.sh` и `scripts/repeat.sh` подключают
  `scripts/require-jdk.sh` и падают раньше — с объяснением и командой починки. Ловится
  именно `JAVA_HOME`: Maven смотрит туда, а не на `java -version`, и у jenv это разные вещи
  (в шелле без инициализации jenv `JAVA_HOME` спокойно остаётся от прошлой версии).
- **JDK НОВЕЕ нужной** (например 26) → компиляция пройдёт, а тесты уйдут на чужую JVM.
  Это ловит канарейка `runsOnExpectedJvm` по `expected.java.feature`. Отдельной проверки в
  скриптах для этого случая нет намеренно — дублировать работающий гейт незачем.

Чинится в обоих случаях одинаково: `jenv local 25` в каталоге проекта.
