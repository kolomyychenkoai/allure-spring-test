...
это не задевает. Если `mvn` вдруг пошёл не на той JVM, канарейка
`runsOnExpectedJvm` покраснеет и скажет об этом прямо; чинится `jenv local 25` в каталоге проекта.
