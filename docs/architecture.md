...
Время: сам документ — минут пятнадцать. Маршрут по коду (§2) — 1301 строка; беглым проходом
это полчаса и даёт картину, вдумчивое чтение с инвариантами — часа полтора.

Числа в документе сняты командами с репозитория; там, где число легко проверить самому,
команда стоит рядом. Часть из них пересчитывает `unit/ArchitectureDocTest`: добавили класс
или модуль, не поправив текст, — сборка краснеет с указанием, какое число разошлось.
...
491 тест. Зависимость у потребителя ровно одна в `compile` (`allure-java-commons`),
...
⚠️ Числа дрейфуют с каждым коммитом. Число тестов — единственное, которого нет под
`ArchitectureDocTest`: точное значение даёт только выполнение сьюта, его печатает команда выше.

---

## 2. Маршрут чтения

Шесть шагов (восемь файлов) по порядку. В колонке — реальный объём, чтобы время каждый оценил
сам; у `AllureAdviceSupport` из 273 строк 124 — комментарии.
...
остальные байткод-модули повторяют шаблон из п. 3. Карта всех точек входа — в §5.
...
Шесть правил, которые повторяются во всех модулях.
...
4. **Гард перед install.** Листенер регистрируется всегда, поэтому модуль сам защищается от
   ...
   большинство модулей спрашивают `ClassPresence.isPresent(…)`
...
списки «что ОК и что нет» — в README, раздел «Параллельный запуск».
...
Канарейка нужна обоим, но ловит разное — стоит ли вообще держать такие точки, вопрос 2 из §12.

---

## 10. Что уже проверено

- **491 тест**, два уровня: A — детерминированные проверки содержимого отчёта через
