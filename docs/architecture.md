...
чтобы спорить по существу, не читая все классы подряд.

Сам документ читается минут пятнадцать. Маршрут по коду из §2 пробегается глазами за полчаса,
и картина уже складывается, а читать вдумчиво, вместе с правилами из §7, придётся часа полтора.

**Посчитанных чисел о самом репозитории здесь нет** — ни сколько классов, ни сколько тестов.
Такое число меняется с каждым коммитом, решений читателю не меняет, а протухает молча:
замер и разбор — в `docs/review-playbook.md`, проход 2.10, ось 4. Где объём правда важен, он назван порядком
величины; где нужно точное — рядом стоит команда, которой его получить.
...
Библиотека небольшая: около семидесяти классов основного кода, меньше восьми тысяч строк,
тестов почти вдевятеро больше, чем классов. У клиентского проекта появляется **ровно одна**
зависимость в `compile` (`allure-java-commons`), все остальные помечены `provided`/`optional`:
модуль включается, только если технология уже есть в тестах.

Точные числа — командой, а не из текста:

```bash
find src/main/java -name '*.java' | wc -l    # классов в src/main
mvn clean test                               # тестов в первом исполнении
```

⚠️ Единственную зависимость в `compile` стережёт `unit/PomCompatibilityTest`: это обещание
потребителю, а не счёт, и его нарушение видно сразу.

---

## 2. В каком порядке читать код

Шесть шагов, восемь файлов, читать по порядку. Весь маршрут — около полутора тысяч строк,
то есть вечер; первые три шага примерно вдвое короче. Комментарии занимают около двух пятых
объёма, так что строки идут быстрее, чем кажется.

| # | файл | что смотреть |
|---|---|---|
| 1 | `internal/AllureInstrumentation` | единственная точка установки агента: `retransform`, аварийный выключатель, поведение при сбое (matcher и advice приносит каждый модуль сам) |
| 2 | `internal/AllureAdviceSupport` | рендер значений: три функции для трёх мест отчёта; почему их три и что ломается при путанице |
| 3 | `assertion/AllureAssertionsListener` + `assertion/internal/AllureAssertJInstrumentation` | типовой байткод-модуль целиком: гард присутствия → install → advice → шаг |
| 4 | `rest/AllureMockMvcAutoConfiguration` + `internal/MovedCustomizerRegistrar` | второй механизм активации и приём «тип резолвится по имени», которым держится кросс-версионность |
| 5 | `data/internal/AllureRepositoryAspect` | Spring AOP, рендер сущностей, защита от побочных эффектов |
| 6 | `internal/InstrumentationDiagnostics` | как видно, что перехват сломался: счётчики трансформаций и сбоев, выборка имён типов (дамп и гейт — на стороне тестов, см. §10) |

Если времени мало, хватит шагов 1, 2 и 3: они дают механику целиком, а остальные
байткод-модули повторяют шаблон из пункта 3. Карту всех точек входа смотри в §5.

Что `unit/ArchitectureDocTest` держит: таблица точек входа из §5 совпадает с ресурсами Spring
**в обе стороны** — и недостающая строка, и лишняя краснеют; маршрут ведёт в существующие
классы; всё, на что документ ссылается, существует. Отдельно `autoconfig/ActivationRegistrationTest`
сверяет с `SpringFactoriesLoader` уже не документ, а свой список: точки входа поднимаются
ровно те, что объявлены.
...
flowchart LR
    ...
    subgraph ACT["Три механизма активации"]
        direction TB
        L["META-INF/spring.factories<br/><b>TestExecutionListener</b>"]
        A["AutoConfiguration.imports<br/><b>автоконфигурации</b>"]
        B["ByteBuddy-агент<br/><b>internal/AllureInstrumentation</b>"]
    end

    T --> L
    T --> A
    L -- "часть листенеров зовёт install()" --> B
...
Байткод ставят только листенеры, и то не все — у автоконфигураций такого пути
...
Листенеры из `META-INF/spring.factories` дают точку входа на каждый тест. Таблица ниже
сверяется с ресурсами Spring в обе стороны (`unit/ArchitectureDocTest`), поэтому лишняя
строка здесь краснеет так же, как недостающая:
...
Автоконфигурации из `AutoConfiguration.imports` становятся бинами:

| автоконфиг | что регистрирует | тонкость |
|---|---|---|
| `rest/AllureMockMvcAutoConfiguration` | кастомайзер MockMvc, который вешает `AllureMockMvcResultHandler` на каждый собираемый `MockMvc` | тип кастомайзера переехал между Boot 3 и 4 — резолвится по имени, бин регистрируется программно из `BeanFactoryPostProcessor`, то есть после ВСЕХ постпроцессоров реестра: иначе поздняя регистрация того же имени у потребителя роняет старт (#73) |
...
## 6. Что лежит в `internal/`

Публичного API тут нет: это внутренний код, и `package-info` это фиксирует.

| класс | зачем |
|---|---|
| `AllureInstrumentation` | `retransform(matcher, transformer)` + аварийный выключатель `-Dallure.spring.instrumentation=off`. Единственное место, где вызывается `ByteBuddyAgent.install()` и строится `AgentBuilder`; сам byte-buddy импортируют и модули — они пишут свои matcher и advice |
...
3. **Никогда не бросать исключение в клиентский код.** Сбой перехвата даёт запись в лог
...
6. **Идемпотентность установки.** У каждого модуля-инструментатора свой
...
- **строковые** (`named("send")`, `named("perform")`) — компилятор их не проверяет,
...
- **Тесты в двух уровнях.** Уровень A — детерминированные проверки содержимого отчёта
...
4. **Границы модулей.** У каждого модуля свой `internal` — кроме `config`.
