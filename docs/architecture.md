...
В `src/main` лежит **69 классов / 7117 строк** в 20 пакетах, в `src/test` — **116 классов**
и 553 тестов. У клиентского проекта появляется ровно одна зависимость в `compile`
...
| 5 | `data/internal/AllureRepositoryAspect` | Spring AOP, рендер сущностей, защита от побочных эффектов | 382 |
...
Итого 1334 строка. Если времени мало, хватит шагов 1, 2 и 3: это 633 строки, и они дают
...
- **553 тестов** в двух уровнях. Уровень A — детерминированные проверки содержимого отчёта
