...
вместе с инвариантами, придётся часа полтора.
...
⚠️ **На схеме модуль стоит под одним механизмом, а в коде их бывает два.** У MockMvc
работают кастомайзер из автоконфигурации и байткод на `perform`, у WireMock — листенер
и байткод на `stubFor`/`verify`/`reset`. Точный механизм по каждой точке входа указан в §5.
...
running». Выглядит этот гейт в разных модулях по-разному, разбор — в §7, инвариант 1.
