...
В `src/main` лежит **71 классов / 7681 строк** в 20 пакетах, в `src/test` — **122 классов**
и 599 тестов. У клиентского проекта появляется ровно одна зависимость в `compile`
...
|---|---|---|---|
| 1 | `internal/AllureInstrumentation` | единственная точка установки агента: `retransform`, аварийный выключатель, поведение при сбое (matcher и advice приносит каждый модуль сам) | 149 |
...
| 4 | `rest/AllureMockMvcAutoConfiguration` + `internal/MovedCustomizerRegistrar` | второй механизм активации и приём «тип резолвится по имени», которым держится кросс-версионность | 46 + 211 |
| 5 | `data/internal/AllureRepositoryAspect` | Spring AOP, рендер сущностей, защита от побочных эффектов | 377 |
| 6 | `internal/InstrumentationDiagnostics` | как видно, что перехват сломался: счётчики трансформаций и сбоев, выборка имён типов (дамп и гейт — на стороне тестов, см. §10) | 237 |

Итого строк: 1514. Если времени мало, хватит шагов 1, 2 и 3: это 643 строки, и они дают
...
- **599 тестов** в двух уровнях. Уровень A — детерминированные проверки содержимого отчёта
