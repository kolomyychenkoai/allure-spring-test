...
В `src/main` лежит **70 классов / 7238 строк** в 20 пакетах, в `src/test` — **118 классов**
и 568 тестов. У клиентского проекта появляется ровно одна зависимость в `compile`
...
| 6 | `internal/InstrumentationDiagnostics` | как видно, что перехват сломался: счётчики трансформаций и сбоев, выборка имён типов (дамп и гейт — на стороне тестов, см. §10) | 132 |

Итого 1343 строка. Если времени мало, хватит шагов 1, 2 и 3: это 642 строки, и они дают
...
- **568 тестов** в двух уровнях. Уровень A — детерминированные проверки содержимого отчёта
