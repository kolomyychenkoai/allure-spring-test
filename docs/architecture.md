...
| 5 | `data/internal/AllureRepositoryAspect` | самый «прикладной» модуль: Spring AOP, рендер сущностей, защита от побочных эффектов | 5 |
| 6 | `internal/InstrumentationDiagnostics` | как видно, что перехват сломался: счётчики, дамп, гейт в сборке | 2 |
...
2. **Отчёт не имеет права менять поведение приложения.** Ни исключением, ни лишним запросом
   в БД, ни вычитанным одноразовым телом ответа. Отсюда гейты, `try/catch` вокруг всего
   рендера и страж ленивых JPA-связей.
...
| механизм | что через него | чем платим |
|---|---|---|
...
Общий приём: **матчеры заданы строками, компилятор их не проверяет**, поэтому на каждое
допущение есть канарейка, которая краснеет ТОЧЕЧНО и говорит, какой матчер чинить.

---

## 10. Что уже проверено (чтобы не тратить на это ревью)
...
- **Диагностика сбоев перехвата** — счётчики трансформаций и дамп; гейт в сборке.
