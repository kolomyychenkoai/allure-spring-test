...
за три месяца больше трети правок этого документа состояли ровно из перенумерации, и при этом
ни одного настоящего дефекта те числа не поймали. Где объём правда важен, он назван порядком
...
тестов вдесятеро больше, чем классов. У клиентского проекта появляется **ровно одна**
...
то есть вечер; первые три шага примерно вдвое короче. Больше половины объёма в каждом файле
занимают комментарии: строки идут медленнее, чем кажется по числу.
...
Что `unit/ArchitectureDocTest` по-прежнему держит: каждый listener и автоконфиг из ресурсов
Spring назван в этом документе, маршрут ведёт в существующие классы, и всё, на что документ
ссылается, существует. Полноту списка листенеров в обе стороны стережёт
`autoconfig/ActivationRegistrationTest` — он требует ТОЧНОГО равенства с `SpringFactoriesLoader`.
...
Листенеры из `META-INF/spring.factories` дают точку входа на каждый тест — все они перечислены
ниже, и полноту списка стережёт `autoconfig/ActivationRegistrationTest`:
