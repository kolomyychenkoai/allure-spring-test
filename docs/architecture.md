...
В `src/main` лежит **71 классов / 7680 строк** в 20 пакетах, в `src/test` — **123 классов**
и 605 тестов. У клиентского проекта появляется ровно одна зависимость в `compile`
...
| 6 | `internal/InstrumentationDiagnostics` | как видно, что перехват сломался: счётчики трансформаций и сбоев, выборка имён типов (дамп и гейт — на стороне тестов, см. §10) | 236 |

Итого строк: 1513. Если времени мало, хватит шагов 1, 2 и 3: это 643 строки, и они дают
...
- **605 тестов** в двух уровнях. Уровень A — детерминированные проверки содержимого отчёта
