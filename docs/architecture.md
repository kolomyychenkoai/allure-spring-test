...
В `src/main` лежит **70 классов / 7322 строк** в 20 пакетах, в `src/test` — **118 классов**
и 573 теста. У клиентского проекта появляется ровно одна зависимость в `compile`
...
| 6 | `internal/InstrumentationDiagnostics` | как видно, что перехват сломался: счётчики трансформаций и сбоев, выборка имён типов (дамп и гейт — на стороне тестов, см. §10) | 203 |

Итого 1414 строка. Если времени мало, хватит шагов 1, 2 и 3: это 642 строки, и они дают
...
| `AllureInstrumentationLogger` | единый канал диагностики байткод-модулей: WARNING на сорванный перехват, FINE на чужой нерезолвнутый тип |
...
- **573 теста** в двух уровнях. Уровень A — детерминированные проверки содержимого отчёта
