...
чтобы спорить по существу, не читая все классы подряд (сколько их — в §1, под гейтом).

Сам документ читается минут пятнадцать. Маршрут по коду из §2 (его объём — там же, под
гейтом) пробегается глазами за полчаса, и картина уже складывается, а читать вдумчиво,
...
В `src/main` лежит **70 классов / 7382 строк** в 20 пакетах, в `src/test` — **118 классов**
и 579 тестов. У клиентского проекта появляется ровно одна зависимость в `compile`
...
⚠️ Числа расходятся с кодом при каждом коммите. Их сторожат два теста: `ArchitectureDocTest`
считает классы и строки по репозиторию, а количество тестов сверяет `inventory/TestCountCheck`
по отчётам первого исполнения — и в этом файле, и в `docs/testing.md`.
...
|---|---|---|---|
| 1 | `internal/AllureInstrumentation` | единственная точка установки агента: `retransform`, аварийный выключатель, поведение при сбое (matcher и advice приносит каждый модуль сам) | 148 |
...
| 4 | `rest/AllureMockMvcAutoConfiguration` + `internal/MovedCustomizerRegistrar` | второй механизм активации и приём «тип резолвится по имени», которым держится кросс-версионность | 49 + 176 |
| 5 | `data/internal/AllureRepositoryAspect` | Spring AOP, рендер сущностей, защита от побочных эффектов | 377 |
| 6 | `internal/InstrumentationDiagnostics` | как видно, что перехват сломался: счётчики трансформаций и сбоев, выборка имён типов (дамп и гейт — на стороне тестов, см. §10) | 238 |

Итого 1482 строка. Если времени мало, хватит шагов 1, 2 и 3: это 642 строки, и они дают
...
- **579 тестов** в двух уровнях. Уровень A — детерминированные проверки содержимого отчёта
