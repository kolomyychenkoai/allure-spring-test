...
пробежать его глазами — это полчаса, и картина уже складывается, а читать вдумчиво,
...
В `src/main` лежит **68 классов / 6346 строк** в 20 пакетах, в `src/test` — **103 класса**
и 491 тест. У клиентского проекта появляется ровно одна зависимость в `compile`
...
не сторожит, — количество тестов: точное значение известно только после прогона,
...
MockMvc — кастомайзер из автоконфигурации плюс байткод на `perform`. WebTestClient —
кастомайзер снимает обмен, а листенер его проигрывает. WireMock — листенер на старте
сервера и запросах плюс байткод на `stubFor`/`verify`/`reset`. RestAssured — фильтр
в `RestAssured.filters` плюс байткод на проверках `.then()`. Точный механизм по каждой
...
установки байткода — из-за этого модуль вроде MockMvc и заводится сразу с двух сторон.
...
- **491 тест** в двух уровнях. Уровень A — детерминированные проверки содержимого отчёта
...
- **Инструменты ревью зависят от `python3`** в пяти местах, issue #50.
