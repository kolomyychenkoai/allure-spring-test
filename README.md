...
- **Kafka `@KafkaListener`** — приём ловится, но привязка к тесту по ОКНУ тест-метода.
  *Почему:* контейнер-слушатель крутит `poll` на своём потоке, где активного Allure-кейса
  нет. Записи буферизуются и проигрываются шагами на тест-потоке в `afterTestMethod` (как у
  WireMock-запросов). Окно привязки = выполнение тест-метода: буфер чистится перед каждым
  тестом. Поэтому сообщение, принятое МЕЖДУ тестами или ПОСЛЕ того как тест перестал ждать
  (тест без `latch.await`/ожидания приёма), приклеится не к тому тесту или потеряется.
  *Что делать:* в тесте дожидайся приёма (`CountDownLatch`/Awaitility) — тогда запись
  попадёт в окно своего теста. Ручной `consumer.poll(...)` в теле теста пишется сразу и под
  это ограничение не попадает.
