...
Единственное свойство, которое имеет смысл менять — состав среза конфигурации:

```yaml
# добавить свой namespace в снимок Configuration
allure:
  spring:
    config:
...
По умолчанию в срез попадают свойства с префиксами `spring.`, `server.`, `logging.`,
`management.`. Значения **не маскируются** — используй фейковые данные в тестах.
