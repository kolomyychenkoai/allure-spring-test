...
- **Параллельный запуск** (`@Execution(CONCURRENT)`) — работает ЧАСТИЧНО.
  ...
  *Как запускать:* шаги — параллельно спокойно. Если важна чистота «Application Logs» или
  используешь глобальный RestAssured — этот класс имеет смысл гонять последовательно (или
  процессами: forked-JVM surefire, у каждого своя JVM). Это про режим запуска, не про тест-код.
