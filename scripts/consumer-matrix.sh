...
#
# Зачем именно так. Библиотека способна сломать тест потребителя одним фактом подключения
# (перехват вычитывал одноразовое тело ответа), а её собственная витрина такого не видит:
# она устроена ПОД неё.
#
...
for svc in "${services[@]}"; do
    ...
    if diff -u "$svc-$MODE-without.snapshot" "$svc-$MODE-with.snapshot" > "$svc-$MODE.diff"; then
        # $tests — из проверки порога выше: снимки совпали, значит число одно на обе стороны
