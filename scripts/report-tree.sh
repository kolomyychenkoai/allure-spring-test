...
# проверялось и почему результат верный. Это видно только на дереве целиком.
# Собирать такое дерево руками дорого, поэтому шаг пропускался — ровно до тех пор,
# пока заказчик не попросил посмотреть отчёт и находки не нашлись сразу.
#
# Что печатает:
